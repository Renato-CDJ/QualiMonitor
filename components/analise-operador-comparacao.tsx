              ...
              <div className="flex min-h-0 flex-col gap-3">
                ...
                <div className="max-h-80 overflow-y-auto pr-1">
                  <div className="flex flex-col gap-2">
                    ...
                    {operadorResumo.map((item) => <button key={item.operador} type="button" onClick={() => setOperadorSelecionado(item.operador)} className={cn("rounded-xl border px-4 py-3 text-left transition-colors", operadorSelecionado === item.operador ? "border-primary bg-primary/10" : "bg-background hover:border-primary/40")}><p className="truncate text-sm font-semibold">{item.operador}</p><p className="mt-1 text-xs text-muted-foreground">{item.qtd} itens avaliados · média {item.media}%</p></button>)}
                  </div>
                </div>
              </div>
              <div className="flex min-h-[180px] items-center justify-center rounded-xl border bg-background/70 p-5 text-center"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Resultado do operador por item</p><p className="mt-2 text-lg font-semibold">{operadorSelecionado === "todos" ? "Todos os operadores" : operadorSelecionado}</p><p className="mt-1 text-sm text-muted-foreground">A tabela abaixo mostra os dados de Tópico, Qtd Itens, % Conforme e % Inconforme.</p></div></div>
            </div>}
