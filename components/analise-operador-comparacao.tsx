...
import { CalendarDays, Check, ChevronRight, Download, Filter, History, Layers, RotateCcw, Search } from "lucide-react"
      ...
      <CardContent>
        <Tabs defaultValue="comparativo" className="flex flex-col gap-5">
          ...
          <TabsContent value="historico" className="mt-0 flex flex-col gap-4">
            <div className="rounded-xl border bg-secondary/20 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="grid flex-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  <div className="flex flex-col gap-1.5"><Label htmlFor="historico-operador" className="text-xs text-muted-foreground">Quem analisar</Label><Select value={operadorFiltro} onValueChange={(value) => setOperadorFiltro(value ?? "todos")}><SelectTrigger id="historico-operador"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="todos">Todos os operadores</SelectItem>{operadores.map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
            ...
            {historicoItens.length === 0 ? <p className="py-16 text-center text-sm text-muted-foreground">Nenhum apontamento encontrado nesse período.</p> : <div className="overflow-x-auto rounded-2xl border border-border/70 bg-card/30 shadow-sm"><div className="min-w-[720px]"><div className="sticky top-0 z-10 grid grid-cols-[minmax(260px,1fr)_repeat(var(--historico-datas),minmax(112px,1fr))] border-b border-border/70 bg-background/95 shadow-sm backdrop-blur" style={{ "--historico-datas": historicoDatas.length } as CSSProperties}><div className="flex min-h-14 items-center gap-2 border-r border-border/50 px-4 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground"><span>Item do checklist</span><Dialog open={filtroItensAberto} onOpenChange={setFiltroItensAberto}><DialogTrigger render={<Button type="button" variant={itensFiltro.size > 0 ? "secondary" : "ghost"} size="icon" className="size-7" aria-label="Filtrar itens do checklist" />}><Filter data-icon="inline-start" /></DialogTrigger><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Filtrar itens do checklist</DialogTitle><DialogDescription>Selecione um ou mais itens para exibir no histórico.</DialogDescription></DialogHeader><div className="flex max-h-80 flex-col gap-1 overflow-y-auto rounded-lg border p-2">{itensDisponiveis.length === 0 ? <p className="px-3 py-6 text-center text-sm text-muted-foreground">Nenhum item disponível para os filtros atuais.</p> : itensDisponiveis.map((item) => { const selecionado = itensFiltro.has(item.itemId); return <button key={item.itemId} type="button" onClick={() => setItensFiltro((prev) => { const next = new Set(prev); selecionado ? next.delete(item.itemId) : next.add(item.itemId); return next })} className="flex items-center gap-3 rounded-md px-3 py-2 text-left text-sm font-normal normal-case tracking-normal transition-colors hover:bg-secondary"><span className={cn("flex size-4 shrink-0 items-center justify-center rounded-sm border", selecionado ? "border-primary bg-primary text-primary-foreground" : "border-input")}>{selecionado && <Check className="size-3" />}</span><span className="min-w-0 flex-1"><span className="block truncate">{item.texto}</span><span className="text-xs text-muted-foreground">{item.bloco}</span></span></button> })}</div><div className="flex justify-between gap-2"><Button type="button" variant="ghost" onClick={() => setItensFiltro(new Set())}>Limpar seleção</Button><Button type="button" onClick={() => setFiltroItensAberto(false)}>Aplicar filtro</Button></div></DialogContent></Dialog>{itensFiltro.size > 0 && <span className="rounded-full bg-primary/15 px-1.5 py-0.5 text-[10px] font-medium normal-case tracking-normal text-primary">{itensFiltro.size}</span>}</div>{historicoDatas.map((data) => <div key={data} className="border-l px-3 py-3 text-center text-xs font-semibold text-muted-foreground">{formatDate(data)}</div>)}</div>{historicoItens.map((item) => <div key={`${item.bloco}-${item.texto}`} className="grid grid-cols-[minmax(260px,1fr)_repeat(var(--historico-datas),minmax(112px,1fr))] border-b last:border-b-0" style={{ "--historico-datas": historicoDatas.length } as CSSProperties}><div className="flex min-h-16 flex-col justify-center px-4 py-2"><p className="text-sm font-medium">{item.texto}</p><p className="text-xs text-muted-foreground">{item.bloco}</p></div>{historicoDatas.map((data) => { const apontamento = item.porData.get(data); return <div key={data} className="flex min-h-16 items-center justify-center border-l px-2 py-2">{apontamento ? <span className={cn("rounded-full border px-2.5 py-1 text-center text-xs font-semibold", apontamento.status === "conforme" ? "border-chart-5/30 bg-chart-5/10 text-chart-5" : apontamento.status === "inconforme" ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-border bg-secondary text-muted-foreground")} title={`${apontamento.monitor} · Nota ${apontamento.nota}`}>{apontamento.status === "conforme" ? "Conforme" : apontamento.status === "inconforme" ? "Inconforme" : "N.A."}</span> : <span className="text-xs text-muted-foreground/40">—</span>}</div>})}</div>)}</div></div>}
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
