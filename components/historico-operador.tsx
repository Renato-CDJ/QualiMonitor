              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <Dialog open={historicoAberto} onOpenChange={setHistoricoAberto}>
                    <DialogTrigger render={<Button variant="secondary" size="sm"><CalendarDays data-icon="inline-start" />Histórico mensal</Button>} />
                    <DialogContent className="w-[60vw] max-w-[60vw] min-w-[720px]">
                      ...
                      {historicoMensal && <div className="flex flex-col gap-5">
                        <div className="grid gap-3 rounded-xl border border-border/60 bg-secondary/20 p-4 sm:grid-cols-2">
                          <label className="flex flex-col gap-1.5 text-xs font-medium text-muted-foreground">Mês de referência<select value={historicoMensal.mes} onChange={(event) => setMesHistorico(event.target.value)} className="h-10 rounded-lg border border-input bg-background px-3 text-sm text-foreground shadow-sm"><option value="" disabled>Selecione o mês</option>{historicoMensal.meses.map((mes) => <option key={mes} value={mes}>{new Intl.DateTimeFormat("pt-BR", { month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${mes}-01T00:00:00Z`))}</option>)}</select></label>
                        ...
                        <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-primary/20 bg-primary/5 p-4"><p className="text-xs font-medium text-muted-foreground">Média do mês</p><p className="mt-1 text-2xl font-semibold tracking-tight text-primary">{historicoMensal.dias.filter((dia) => dia.media !== null).length ? Math.round(media(historicoMensal.dias.flatMap((dia) => dia.media === null ? [] : [dia.media])) * 10) / 10 : "—"}</p></div><div className="rounded-xl border border-border/60 bg-secondary/30 p-4"><p className="text-xs font-medium text-muted-foreground">Monitorias</p><p className="mt-1 text-2xl font-semibold tracking-tight">{historicoMensal.monitorias}</p></div><div className="rounded-xl border border-border/60 bg-secondary/30 p-4"><p className="text-xs font-medium text-muted-foreground">Dias avaliados</p><p className="mt-1 text-2xl font-semibold tracking-tight">{historicoMensal.diasComDados}<span className="ml-1 text-sm font-normal text-muted-foreground">/{historicoMensal.dias.length}</span></p></div></div>
                        <div className="rounded-xl border border-border/60 bg-card/40 p-3"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-semibold capitalize">{new Intl.DateTimeFormat("pt-BR", { month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${historicoMensal.mes}-01T00:00:00Z`))}</p><div className="flex items-center gap-2 text-[11px] text-muted-foreground"><span className="size-2 rounded-full bg-chart-5" />90+<span className="size-2 rounded-full bg-chart-1" />75–89<span className="size-2 rounded-full bg-destructive" />&lt;60</div></div><div className="grid grid-cols-7 gap-1.5 text-center text-xs"><div className="pb-1 font-medium text-muted-foreground">Dom</div><div className="pb-1 font-medium text-muted-foreground">Seg</div><div className="pb-1 font-medium text-muted-foreground">Ter</div><div className="pb-1 font-medium text-muted-foreground">Qua</div><div className="pb-1 font-medium text-muted-foreground">Qui</div><div className="pb-1 font-medium text-muted-foreground">Sex</div><div className="pb-1 font-medium text-muted-foreground">Sáb</div>{Array.from({ length: historicoMensal.primeiroDia }).map((_, index) => <div key={`empty-${index}`} />)}{historicoMensal.dias.map((dia) => <div key={dia.data} title={dia.media === null ? "Sem monitorias" : `${dia.media} pontos · ${dia.quantidade} monitoria(s)`} className={cn("flex min-h-16 flex-col items-center justify-center rounded-lg border text-xs transition-all hover:-translate-y-0.5 hover:shadow-sm", dia.media === null ? "border-border/50 bg-background text-muted-foreground/40" : dia.media >= 90 ? "border-chart-5/40 bg-chart-5/20 text-chart-5" : dia.media >= 75 ? "border-chart-1/40 bg-chart-1/20 text-chart-1" : dia.media >= 60 ? "border-chart-3/40 bg-chart-3/20 text-chart-3" : "border-destructive/40 bg-destructive/20 text-destructive")}><span className="font-medium">{dia.dia}</span>{dia.media !== null && <span className="mt-0.5 font-semibold">{dia.media}</span>}</div>)}</div>
                      </div></div>}
                    </DialogContent>
                  </Dialog>
