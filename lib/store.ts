...
const SEED_CARTEIRAS: Carteira[] = CARTEIRAS.map((nome, i) => ({
  id: `carteira-${i + 1}`,
  nome,
  descricao: `Operação de ${nome.toLowerCase()}`,
  ativa: true,
...
async function seedInicial() {
  ...
  cache.carteiras = SEED_CARTEIRAS
...
async function hydrate(): Promise<void> {
  ...
  hydrating = (async () => {
    ...
    if (bancoVazio) {
      ...
      cache.carteiras = SEED_CARTEIRAS.filter((c) => cache.checklists.some((checklist) => checklist.carteira === c.nome))
...
export const store = {
  ...
  getCarteiras: () => cache.carteiras,
  setCarteiras: (v: Carteira[]) => {
    cache.carteiras = v
    emitUpdate("carteiras")
  },
  addCarteira: (carteira: Carteira) => {
    cache.carteiras = [carteira, ...cache.carteiras]
    emitUpdate("carteiras")
  },
